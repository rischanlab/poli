...
class grocery_CRUD_Model  extends CI_Model  {
		...
		if(!empty($this->relation))
			foreach($this->relation as $relation)
			{
				...
				if(strstr($related_field_title,'{'))
					...
					$select .= ", '".str_replace(array('{','}'),array("'||COALESCE({$unique_join_name}.",", '')||'"),str_replace("'","\\'",$related_field_title))."' as $unique_field_name";
				else
	...
	function join_relation($field_name , $related_table , $related_field_title)
	{
		...
		if($related_primary_key !== false)
		{
			...
			$this->db->join( $related_table.' "'.$unique_name.'"' , "$unique_name.$related_primary_key = {$this->table_name}.$field_name",'left');
	...
	function get_field_types_basic_table()
	{
		...
		$sql = "SELECT X.CONSTRAINT_TYPE,X.OWNER,A.* FROM user_tab_cols A left outer join ( SELECT Distinct  C.CONSTRAINT_TYPE,A.COLUMN_NAME,C.OWNER  FROM ALL_CONS_COLUMNS A JOIN ALL_CONSTRAINTS C  ON A.CONSTRAINT_NAME = C.CONSTRAINT_NAME WHERE C.TABLE_NAME = '{$this->table_name}' AND C.CONSTRAINT_TYPE = 'P'  AND C.OWNER='{$this->db->username}')  X on A.COLUMN_NAME = X.COLUMN_NAME WHERE A.table_name = '{$this->table_name}'";


		foreach($this->db->query($sql)->result() as $db_field_type)

		{
			$type = explode("(",$db_field_type->DATA_TYPE);
			$db_type = $type[0];
			/**
			 if(isset($type[1]))
			 {
			 ...
			 **/
			$length =$db_field_type->DATA_LENGTH;
				
			$db_field_types[$db_field_type->COLUMN_NAME]['db_max_length'] = $length;
			$db_field_types[$db_field_type->COLUMN_NAME]['db_type'] = $db_type;
			$db_field_types[$db_field_type->COLUMN_NAME]['db_null'] =  $db_field_type->NULLABLE == 'Y' ? true : false;
			$db_field_types[$db_field_type->COLUMN_NAME]['db_extra'] = '';// $db_field_type->Extra;
			$db_field_types[$db_field_type->COLUMN_NAME]['CONSTRAINT_TYPE'] =$db_field_type->CONSTRAINT_TYPE;// $db_field_type->Extra;
		}
	...
	function get_field_types($table_name)
	{
		...
		$sql = "SELECT X.CONSTRAINT_TYPE,X.OWNER,A.* FROM user_tab_cols A left outer join ( SELECT Distinct  C.CONSTRAINT_TYPE,A.COLUMN_NAME,C.OWNER  FROM ALL_CONS_COLUMNS A JOIN ALL_CONSTRAINTS C  ON A.CONSTRAINT_NAME = C.CONSTRAINT_NAME WHERE C.TABLE_NAME = '{$table_name}' AND C.CONSTRAINT_TYPE = 'P'  AND C.OWNER='{$this->db->username}')  X
		on A.COLUMN_NAME = X.COLUMN_NAME
	...
	function db_insert($post_array)
	{
		...
		$sql = "select {$this->table_name}_SEQ.nextval AS NEXTID from dual";
		$query = $this->db->query($sql);
		$row = $query->row();
		$newid= $row->NEXTID;
		$pkfield = $this->get_primary_key();
		$post_array[$pkfield]  = $newid;
	...
	function db_delete($primary_key_value)
	{
		...
		/**
		 $xxx =  implode(",",array( $primary_key_field => $primary_key_value));
		 $xxx .= $this->table_name;
